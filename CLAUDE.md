...
- **Skill files** (`SKILL.md`): YAML frontmatter with `name` and `description`, followed by the prompt body. Skills are numbered (`1-scan`, `2-plot`, `3-engage`) to indicate execution order.
- Skills are invoked as `/picard:scan`, `/picard:plot`, `/picard:engage`.

## The picard plugin

Three-phase agentic coding workflow — **never write code until a written plan is approved**:

1. `/picard:scan` — Research only. Produces `doc/work-sessions/<yyyy>/<yyyy-mm-dd>-scan-<desc>.md`. No solutions proposed.
2. `/picard:plot` — Proposal with TDD checklist. Produces `doc/work-sessions/<yyyy>/<yyyy-mm-dd>-proposal-<desc>.md`. Iterates with user until approved.
3. `/picard:engage` — Executes the approved checklist one item at a time, marking each `[x]` in the proposal document as it completes.
